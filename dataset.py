...
class CamLocDataset(Dataset):
    ...
    def _load_pts3D(self, idx):
        fname = self.rgb_files[idx].name.replace('_', '/').split('.')[0]
        img_id = self.sfm_fname2img_id[fname]
        pts_ids = self.sfm_img_id2pts_ids[img_id]
        pts3D = self.sfm_pts3d_arr[pts_ids]
    ...
    def _project_pts3D_to_2D(self, pts3D, pose_inv, intrinsics):
        ...
        behind_camera_mask = pts2D[:, 2] <= 0
        pts2D = pts2D[~behind_camera_mask]
        # TODO: consider clip to 0.1m to avoid numerical issues
        pts2D = pts2D / pts2D[:, 2:]
        pts2D = pts2D[:, :2]  # N, 2
        return pts2D
    ...
    def _get_single_item(self, idx, image_height):
        ...
        if self.augment:
            ...
            image_mask_pts2D = self._rotate_image(image_mask_pts2D, angle, order=1, mode='constant')
        ...
        image_mask_pts2D = image_mask_pts2D > 0
        ...
        return image, image_mask, image_mask_pts2D, pose, pose_inv, intrinsics, intrinsics_inv, coords, str(self.rgb_files[idx]), info
