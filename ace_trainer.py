...
class TrainerACE:
    def __init__(self, options):
        ...
        self.mask_method = options.mask_method
    ...
    def create_training_buffer(self):
        ...
        with torch.no_grad():
            ...
            pbar = tqdm(total=self.options.training_buffer_size)
            while buffer_idx < self.options.training_buffer_size:
                dataset_passes += 1
                for (image_B1HW, image_mask_B1HW, image_mask_pts2D_B1HW,
                     gt_pose_B44, gt_pose_inv_B44,
                     intrinsics_B33, intrinsics_inv_B33,
                     _, _, info) in training_dataloader:

                    # Copy to device.
                    image_B1HW = image_B1HW.to(self.device, non_blocking=True)
                    image_mask_B1HW = image_mask_B1HW.to(self.device, non_blocking=True)
                    image_mask_pts2D_B1HW = image_mask_pts2D_B1HW.to(self.device, non_blocking=True)
                    ...
                    image_mask_B1HW = TF.resize(
                        image_mask_B1HW, [H, W], interpolation=TF.InterpolationMode.NEAREST)
                    image_mask_pts2D_B1HW = TF.resize(
                        image_mask_pts2D_B1HW, [H, W], interpolation=TF.InterpolationMode.NEAREST)
                    image_mask_B1HW = image_mask_B1HW.bool()
                    image_mask_pts2D_B1HW = image_mask_pts2D_B1HW.bool()

                    # If the current mask has no valid pixels, continue.
                    if image_mask_B1HW.sum() == 0 or image_mask_pts2D_B1HW.sum() == 0:
                        continue
                    ...
                    image_mask_pts2D_B1HW = image_mask_pts2D_B1HW.float()
                    image_mask_N1 = normalize_shape(image_mask_B1HW)
                    image_mask_pts2D_N1 = normalize_shape(image_mask_pts2D_B1HW)

                    image_mask_N1 = image_mask_N1 * image_mask_pts2D_N1
                    ...
                    sample_idxs = torch.multinomial(
                        ...
                        features_to_select,
                        replacement=True if features_to_select > image_mask_N1.sum().item() else False,
    ...
    def training_step(self, features_bC, target_px_b2, gt_inv_poses_b34, Ks_b33, invKs_b33):
        ...
        """
        batch_size = features_bC.shape[0]  # 5120
        channels = features_bC.shape[1]   # 512

        # NOTE:  regressor use 1x1 conv, so the input shape do not matter
